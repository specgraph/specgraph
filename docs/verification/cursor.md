...
> **Note:** This file is generated by `specgraph init` from `.specgraph.yaml`
> and global config. Running init updates managed fields (URL, Authorization
> header, X-Specgraph-Project) while preserving siblings and user
> customizations. The URL below reflects the local `default_server` value used
> during verification; fresh installs may resolve to a different configured
> server, such as the current `9090` default.
