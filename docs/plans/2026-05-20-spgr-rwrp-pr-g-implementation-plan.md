...
```bash
cd ~/Code/github.com/specgraph-pr-g
...
Nine commits (Tasks 1, 1a, 2-8). Each commit must stay green under `task check`. The pr-g jj workspace at `~/Code/github.com/specgraph-pr-g` holds the stack. After landing, push the bookmark with `jj git push --bookmark spgr-rwrp-pr-g` and open the PR following the same flow PRs C/D/E/F used.
