...
Run: `cd ~/Code/github.com/specgraph && go test -run TestValidateManifestEntry ./internal/config/managedfiles/`
...
Run: `cd ~/Code/github.com/specgraph && grep -n "init:start\|init:start" internal/config/managedfiles/sentinel_test.go`
...
Run: `cd ~/Code/github.com/specgraph && readlink plugin/opencode/.opencode/plugins/specgraph.ts`
...
```bash
cd ~/Code/github.com/specgraph/plugin/cursor/.cursor/rules
...
# Confirm the right gh account: seanb4t has write access; the corporate SSO account does not.
