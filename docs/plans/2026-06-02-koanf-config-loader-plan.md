...
Each task ends with `jj --no-pager commit`. All commit messages MUST include the DCO trailer `Signed-off-by: Sean Brandt <4678+seanb4t@users.noreply.github.com>` (repo requires it). Run `jj --no-pager commit -m "<msg>"` — it finalizes the working copy and opens a fresh one.
