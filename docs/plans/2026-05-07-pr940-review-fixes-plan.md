...
**Branch context:** Working in jj workspace `deprecate-inject` at `/Users/SeBrandt/Code/github.com/specgraph-deprecate-inject`, descended from bookmark `spgr-yyjf-deprecate-inject` (PR #940). Each phase produces one or more commits via `jj commit -m`.

**Commit-message convention:** Conventional commits + DCO sign-off (`Signed-off-by: Sean Brandt <SeBrandt@geico.com>`). The pre-commit hooks enforce both.
...
Run: `cd /Users/SeBrandt/Code/github.com/specgraph-deprecate-inject && go test ./internal/config/pointers/ -run TestSync_PreservesExistingFileMode -v`
...
Run: `cd /Users/SeBrandt/Code/github.com/specgraph-deprecate-inject && git check-ignore -v AGENTS.md.lock .cursor/rules/specgraph-bootstrap.md.lock 2>&1 || true`
...
Run: `cd /Users/SeBrandt/Code/github.com/specgraph-deprecate-inject && task check`
