...
**Working directory:** `~/Code/github.com/specgraph` (or `/Volumes/Code/github.com/specgraph`, same files via symlink). All `task` and `go` commands run from project root.
...
- Commit message format: `feat(managedfiles): <subject>` or `chore(managedfiles): <subject>`; body references `spgr-rwrp`; trailer carries `Signed-off-by: Sean Brandt <4678+seanb4t@users.noreply.github.com>`.
