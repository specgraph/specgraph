...
func PassesForStage(stage string, posture specv1.Posture) []string {
	...
	for _, cfg := range configs {
		if containsPosture(cfg.autoIn, posture) {
...
func OfferedPasses(stage string, posture specv1.Posture) []string {
	...
	var result []string
	for _, cfg := range configs {
		if containsPosture(cfg.offeredIn, posture) {
			result = append(result, cfg.pass)
		}
	}
	return result
...
// containsPosture reports whether ps contains p.
func containsPosture(ps []specv1.Posture, p specv1.Posture) bool {
	for _, v := range ps {
		if v == p {
			return true
		}
