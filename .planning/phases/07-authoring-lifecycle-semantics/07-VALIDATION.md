...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 07-01-T1 | 07-01 | 1 | LIFE-01 | — | Proto field addition (non-colliding field 3); no wire-surface change | build/proto | `grep -q 'string reason = 3;' proto/specgraph/v1/lifecycle.proto && task proto && grep -q 'Reason' gen/specgraph/v1/lifecycle.pb.go && go build ./...` | ✅ existing (lifecycle.proto, gen/) | ✅ green |
| 07-01-T2 | 07-01 | 1 | LIFE-01 | T-07-03, T-07-04 | Reason threaded via parameterized pgx inside RunInTransaction; errors sanitized (assert connect codes) | unit | `go build ./... && go vet ./... && go test ./internal/storage/... ./internal/server/...` | ✅ existing (lifecycle_handler_test.go) | ✅ green |
| 07-01-T3 | 07-01 | 1 | LIFE-01 | T-07-02 | CLI `--reason` optional flag; no new authz surface | build/smoke | `go build ./... && go run ./cmd/specgraph supersede --help 2>&1 \| grep -q -- '--reason'` | ✅ existing (cmd/specgraph/lifecycle.go) | ✅ green |
| 07-02-T1 | 07-02 | 2 | LIFE-01 | T-07-01 | Stale-lease removal: claims row + CLAIMED_BY edge deleted in same amend tx (txCtx, parameterized pgx) | build | `go build ./... && go vet ./...` | ✅ existing (postgres/lifecycle.go) | ✅ green |
| 07-02-T2 | 07-02 | 2 | LIFE-01, LIFE-02 | T-07-01, T-07-06 | Integration proof of claim-release, re-entry landing, done-only supersede, supersede reason | integration | `go build -tags integration ./internal/storage/postgres/ && task test:integration` | 🆕 `TestLifecycleAmend_ReleasesClaim` added in this task (postgres/lifecycle_test.go) | ✅ green |
| 07-03-T1 | 07-03 | 3 | LIFE-01, LIFE-02 | T-07-04 | `IsValidReEntryStage` allowlist enforced in TransitionAmend handler + LifecycleAmendSpec storage; approved/in_progress/review/done rejected with CodeInvalidArgument (error code, not message); explicit `IsValidReEntryStage(approved)==false` assertion + extended `AmendSpec_InvalidReEntryStage` storage integration case (approved/in_progress/review) guard against a footgun/revert | unit + integration | `go build ./... && go vet ./... && go test ./internal/storage/... ./internal/server/...` (unit); `task test:integration` at wave boundary for extended `AmendSpec_InvalidReEntryStage` | 🆕 `IsValidReEntryStage` + handler rejection test + extended `AmendSpec_InvalidReEntryStage` added in this task (spec_domain.go, spec_domain_test.go, lifecycle_handler.go, lifecycle_handler_test.go, postgres/lifecycle.go, postgres/lifecycle_test.go) | ✅ green |
| 07-03-T2 | 07-03 | 3 | LIFE-01, LIFE-02 | T-07-05, T-07-04, T-07-03 | Reroute inherits handler preconditions; tool passes re_entry_stage through to the single (now-correct) gate | build/grep | `go build ./... && rg -q 're_entry_stage' internal/mcp/tools_authoring.go && rg -q 'new_slug' internal/mcp/tools_authoring.go && rg -q 'Lifecycle.TransitionAmend' internal/mcp/tools_authoring.go` | ✅ existing (mcp/tools_authoring.go) | ✅ green |
| 07-03-T3 | 07-03 | 3 | LIFE-01, LIFE-02 | T-07-03 | Tool tests assert res.IsError / connect codes (not message strings); mock returns sentinels | unit | `go build ./... && go vet ./... && go test ./internal/mcp/...` | ✅ existing (mcp/tools_authoring_test.go) | ✅ green |
...
| 07-04-T2 | 07-04 | 4 | LIFE-01 | T-07-07 | Divergent second amend/supersede impl deleted (no re-divergence); absence greps + green check | build/check | `task proto && go build ./... && go vet ./... && task check` | ✅ existing (authoring_handler.go, storage/*, authoring.proto) | ✅ green |
| 07-05-T1 | 07-05 | 5 | LIFE-01, LIFE-02 | T-07-09 | Skills teach land-one-before + constrained params so MCP agent cannot reproduce #899 no-op | skills-validate | `rg -q 're_entry_stage' internal/mcp/skills/embedded/specgraph-authoring/SKILL.md && rg -q 'new_slug' internal/mcp/skills/embedded/specgraph-authoring/SKILL.md && task skills:validate` | ✅ existing (specgraph-authoring/troubleshooting SKILL.md) | ✅ green |
| 07-05-T2 | 07-05 | 5 | LIFE-01, LIFE-02 | T-07-09, T-07-03 | MCP-only e2e proves in-flight amend + re-author and done→supersede across distinct specs + both rejection cases; asserts res.IsError | e2e | `go build -tags e2e ./e2e/api/ && task test:e2e` | 🆕 `e2e/api/mcp_only_lifecycle_test.go` created in this task | ✅ green |
...
Post-execution Nyquist audit (State A). Every mapped task cross-referenced against the
executed test suite and re-run green.

| Metric | Count |
|--------|-------|
| Requirements audited | 2 (LIFE-01, LIFE-02) |
| Tasks mapped | 12 |
| COVERED | 12 |
| PARTIAL | 0 |
| MISSING | 0 |
...
**Suite status at audit time:** `go test ./internal/...` green (exit 0); postgres integration
(`-tags integration`) green; MCP-only e2e (`-tags e2e`) green (confirmed under Docker).

**Coverage expanded beyond the plan.** The deep code review (`07-REVIEW.md`) and its fixes
(`07-REVIEW-FIX.md`) added six tests not present in the plan-time map, all green:

| Test | Covers | Origin |
|------|--------|--------|
| `TestStoreDecomposeOutput_ReconcilesOnReauthor` (authoring_test.go) | CR-01 — amend→re-decompose slice reconciliation (data-integrity) | deep review |
| `TestLifecycleAbandon_ReleasesClaim` (lifecycle_test.go) | WR-01 — abandon-path claim/lease release (T-07-01 sibling) | deep review |
| `SupersedeSpec_TerminalState` (lifecycle_test.go) | IN-01 — terminal-vs-not-done error distinction | deep review |
| `SupersedeSpec_ConcurrentMutual_NoDeadlock` (lifecycle_test.go) | IN-03 — supersede lock-order / retryable-abort | deep review |
| `TestAuthorTool_Amend_SparkReEntry` (tools_authoring_test.go) | WR-01 (standard) — spark re-entry next-step hint | standard review |
| `TestLifecycleHandler_AcknowledgeDrift_MissingNote` (lifecycle_handler_test.go) | WR-02 (standard) — required-note enforcement | standard review |

No gaps remain; no test generation required. `nyquist_compliant: true` upheld.
...
**Approval:** verified 2026-07-15 — all 12 mapped tasks COVERED (green), 0 gaps, coverage expanded by 6 review-added tests.
