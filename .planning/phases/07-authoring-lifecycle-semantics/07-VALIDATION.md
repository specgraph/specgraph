...
| 07-03-T1 | 07-03 | 2 | LIFE-01, LIFE-02 | T-07-05, T-07-04, T-07-03 | Reroute inherits handler preconditions; tool re-validates nothing (pass-through re_entry_stage) | build/grep | `go build ./... && rg -q 're_entry_stage' internal/mcp/tools_authoring.go && rg -q 'new_slug' internal/mcp/tools_authoring.go && rg -q 'Lifecycle.TransitionAmend' internal/mcp/tools_authoring.go` | ✅ existing (mcp/tools_authoring.go) | ⬜ pending |
| 07-03-T2 | 07-03 | 2 | LIFE-01, LIFE-02 | T-07-03 | Tool tests assert res.IsError / connect codes (not message strings); mock returns sentinels | unit | `go build ./... && go vet ./... && go test ./internal/mcp/...` | ✅ existing (mcp/tools_authoring_test.go) | ⬜ pending |
| 07-04-T1 | 07-04 | 3 | LIFE-01 | T-07-08 | RPC removed from wire surface; remaining funnel RPCs still scope-enforced; build stays green | build/unit | `task proto && go build ./... && go vet ./... && go test ./internal/server/... ./internal/mcp/...` | ✅ existing (authoring.proto, gen/) | ⬜ pending |
| 07-04-T2 | 07-04 | 3 | LIFE-01 | T-07-07 | Divergent second amend/supersede impl deleted (no re-divergence); absence greps + green check | build/check | `task proto && go build ./... && go vet ./... && task check` | ✅ existing (authoring_handler.go, storage/*, authoring.proto) | ⬜ pending |
| 07-05-T1 | 07-05 | 4 | LIFE-01, LIFE-02 | T-07-09 | Skills teach land-one-before + constrained params so MCP agent cannot reproduce #899 no-op | skills-validate | `rg -q 're_entry_stage' internal/mcp/skills/embedded/specgraph-authoring/SKILL.md && rg -q 'new_slug' internal/mcp/skills/embedded/specgraph-authoring/SKILL.md && task skills:validate` | ✅ existing (specgraph-authoring/troubleshooting SKILL.md) | ⬜ pending |
| 07-05-T2 | 07-05 | 4 | LIFE-01, LIFE-02 | T-07-09, T-07-03 | MCP-only e2e proves done→amend→re-author→supersede + both rejection cases; asserts res.IsError | e2e | `go build -tags e2e ./e2e/api/ && task test:e2e` | 🆕 `e2e/api/mcp_only_lifecycle_test.go` created in this task | ⬜ pending |
...
Existing test infrastructure covers all phase requirements. Two new tests are created inside their own plans (not as separate Wave 0 scaffolding):
